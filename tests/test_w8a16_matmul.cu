...
TEST_F(W8A16MatMulTest, SmallMatrixCorrectness) {
    ...
    // Run W8A16 kernel
    w8a16_matmul(d_input.data(), d_weight.data(), d_scales.data(), 
                 d_output.data(), M, N, K, group_size);
    
    // Dequantize and run FP16 reference
    dequantize_weights(d_weight.data(), d_scales.data(), d_weight_fp16.data(), 
                       K, N, group_size);
    fp16_matmul_reference(d_input.data(), d_weight_fp16.data(), 
                          d_output_ref.data(), M, N, K);
...
// Feature: tiny-llm-inference-engine, Property 1: W8A16 MatMul Numerical Accuracy
