...
protected:
    ...
    KVCacheConfig createConfig(int num_layers = 2, int num_heads = 4, 
                                int head_dim = 64, int max_seq_len = 128,
...
RC_GTEST_FIXTURE_PROP(KVCachePropertyTest, AllocationInvariant,
    ...
    RC_ASSERT(cache.getActiveSequenceCount() == static_cast<int>(allocated_ids.size()));
}
...
RC_GTEST_FIXTURE_PROP(KVCachePropertyTest, MemoryInvariant,
                      (int num_ops_raw)) {
    ...
    for (int i = 0; i < num_ops; ++i) {
        ...
        // Random operation
        if (cache.getActiveSequenceCount() < config.max_batch_size && (i % 2 == 0)) {
