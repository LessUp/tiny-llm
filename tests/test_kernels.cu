...
TEST_F(RMSNormTest, BasicCorrectness) {
  ...
  rmsnorm(d_input.data(), d_weight.data(), d_output.data(), batch_size,
          hidden_dim, eps);
...
RC_GTEST_FIXTURE_PROP(RMSNormPropertyTest, InPlaceEquivalence,
  ...
  rmsnorm(d_input1.data(), d_weight.data(), d_output.data(), batch_size,
          hidden_dim, eps);
  ...
  rmsnorm_inplace(d_input2.data(), d_weight.data(), batch_size, hidden_dim,
                  eps);
...
TEST_F(AttentionTest, BasicDecodeAttention) {
  ...
  EXPECT_NO_THROW({
    attention_decode(d_query.data(), d_k_cache.data(), d_v_cache.data(),
                     d_output.data(), scale, batch_size, num_heads, seq_len,
                     head_dim);
    cudaDeviceSynchronize();
  });
...
RC_GTEST_FIXTURE_PROP(AttentionPropertyTest, CausalMaskZerosFuturePositions,
  ...
  auto query =
      randomFP16(batch_size * num_heads * seq_len * head_dim, 1.0f, seed);
  auto key =
      randomFP16(batch_size * num_heads * seq_len * head_dim, 1.0f, seed + 1);
  ...
  get_attention_weights(d_query.data(), d_key.data(), d_weights.data(), scale,
