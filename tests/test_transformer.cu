...
TEST_F(TransformerTest, AttentionDecodeVsPrefillSingleToken) {
    ...
    attention_decode(d_query.data(), d_k_cache.data(), d_v_cache.data(),
                    d_output_decode.data(), scale, batch_size, num_heads, seq_len, head_dim);
...
// Feature: tiny-llm-inference-engine, Property 5: Incremental Decoding Equivalence
...
RC_GTEST_FIXTURE_PROP(TransformerPropertyTest, IncrementalDecodingEquivalence,
    ...
    for (int h = 0; h < num_heads; ++h) {
        for (int d = 0; d < head_dim; ++d) {
            output_full_last[h * head_dim + d] = output_full[(h * seq_len + last_pos) * head_dim + d];
        }
    }
    
    // Property: Incremental decode output should match full prefill output for last position
...
RC_GTEST_FIXTURE_PROP(TransformerPropertyTest, KVCachePreservesData,
    ...
    for (int layer = 0; layer < num_layers; ++layer) {
        k_data_per_layer[layer] = randomFP16(num_tokens * num_heads * head_dim, 1.0f, seed + layer * 2);
        v_data_per_layer[layer] = randomFP16(num_tokens * num_heads * head_dim, 1.0f, seed + layer * 2 + 1);
        
        d_k_buffers.emplace_back(k_data_per_layer[layer].size());
        d_v_buffers.emplace_back(v_data_per_layer[layer].size());
        d_k_buffers.back().copyFromHost(k_data_per_layer[layer].data(), k_data_per_layer[layer].size());
        d_v_buffers.back().copyFromHost(v_data_per_layer[layer].data(), v_data_per_layer[layer].size());
    }
    cudaDeviceSynchronize();
    ...
    for (int layer = 0; layer < num_layers; ++layer) {
        cache.appendKV(seq_id, layer, d_k_buffers[layer].data(), d_v_buffers[layer].data(), num_tokens);
    }
    cudaDeviceSynchronize();
    ...
    for (int layer = 0; layer < num_layers; ++layer) {
        ...
        cudaMemcpy(k_retrieved.data(), k_cache, k_retrieved.size() * sizeof(half), cudaMemcpyDeviceToHost);
        cudaMemcpy(v_retrieved.data(), v_cache, v_retrieved.size() * sizeof(half), cudaMemcpyDeviceToHost);
