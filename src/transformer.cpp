...
TransformerLayer::TransformerLayer(int layer_idx,
...
TransformerLayer::TransformerLayer(TransformerLayer &&other) noexcept
    : layer_idx_(other.layer_idx_), weights_(other.weights_),
      config_(other.config_), norm_output_(other.norm_output_),
      q_buf_(other.q_buf_), k_buf_(other.k_buf_), v_buf_(other.v_buf_),
      attn_output_(other.attn_output_), ffn_gate_(other.ffn_gate_),
...
void TransformerLayer::forward(half *hidden_states, KVCacheManager &kv_cache,
  ...
  rmsNorm(hidden_states, weights_.rms_att_weight, norm_output_, num_tokens,
          stream);
  attention(norm_output_, attn_output_, kv_cache, seq_id, position, num_tokens,
            stream);
  kernels::add_inplace(hidden_states, attn_output_, num_tokens * hidden_dim,
                       stream);

  // FFN sublayer with residual
  // x = x + ffn(rms_norm(x))
  rmsNorm(hidden_states, weights_.rms_ffn_weight, norm_output_, num_tokens,
          stream);
  feedForward(norm_output_, ffn_output_, num_tokens, stream);
  kernels::add_inplace(hidden_states, ffn_output_, num_tokens * hidden_dim,
                       stream);
}
...
void TransformerLayer::forwardPrefill(half *hidden_states,
  ...
  rmsNorm(hidden_states, weights_.rms_att_weight, norm_output_, num_tokens,
          stream);
  attention(norm_output_, attn_output_, kv_cache, seq_id, 0, num_tokens,
            stream);
  kernels::add_inplace(hidden_states, attn_output_, num_tokens * hidden_dim,
                       stream);
...
void TransformerLayer::attention(const half *x, half *output,
                                 KVCacheManager &kv_cache, int seq_id,
  ...
  kernels::w8a16_matmul(x, weights_.wq.data, weights_.wq.scales, q_buf_,
                        num_tokens, num_heads * head_dim, hidden_dim,
                        group_size, stream);

  // K projection: [num_tokens, hidden_dim] @ [hidden_dim, num_kv_heads *
  // head_dim]
  kernels::w8a16_matmul(x, weights_.wk.data, weights_.wk.scales, k_buf_,
                        num_tokens, num_kv_heads * head_dim, hidden_dim,
                        group_size, stream);

  // V projection: [num_tokens, hidden_dim] @ [hidden_dim, num_kv_heads *
  // head_dim]
  kernels::w8a16_matmul(x, weights_.wv.data, weights_.wv.scales, v_buf_,
                        num_tokens, num_kv_heads * head_dim, hidden_dim,
                        group_size, stream);
  ...
  // head_dim, hidden_dim]
  kernels::w8a16_matmul(attn_output_, weights_.wo.data, weights_.wo.scales,
                        output, num_tokens, hidden_dim, num_heads * head_dim,
                        group_size, stream);
}
...
void TransformerLayer::feedForward(const half *x, half *output, int num_tokens,
  ...
  kernels::w8a16_matmul(x, weights_.w1.data, weights_.w1.scales, ffn_gate_,
                        num_tokens, intermediate_dim, hidden_dim, group_size,
                        stream);

  // Up projection: [num_tokens, hidden_dim] @ [hidden_dim, intermediate_dim]
  kernels::w8a16_matmul(x, weights_.w3.data, weights_.w3.scales, ffn_up_,
                        num_tokens, intermediate_dim, hidden_dim, group_size,
                        stream);

  // SiLU activation and element-wise multiply
  kernels::silu_mul_inplace(ffn_gate_, ffn_up_, num_tokens * intermediate_dim,
                            stream);

  // Down projection: [num_tokens, intermediate_dim] @ [intermediate_dim,
  // hidden_dim]
  kernels::w8a16_matmul(ffn_gate_, weights_.w2.data, weights_.w2.scales, output,
                        num_tokens, hidden_dim, intermediate_dim, group_size,
                        stream);
}

void TransformerLayer::rmsNorm(const half *x, const half *weight, half *output,
