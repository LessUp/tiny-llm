...
void Logger::init(LogLevel level, const std::string &log_file, bool async) {
  ...
  if (async) {
    ...
    logger_ = std::make_shared<spdlog::async_logger>(
        "tiny_llm", sinks.begin(), sinks.end(), spdlog::thread_pool(),
        spdlog::async_overflow_policy::block);
  } else {
    logger_ = std::make_shared<spdlog::logger>("tiny_llm", sinks.begin(),
                                                sinks.end());
  }
  ...
  TLLM_DEBUG("Logger initialized: level={}, async={}, file={}",
             static_cast<int>(level), async,
