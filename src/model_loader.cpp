...
Result<ModelWeights> ModelLoader::loadBin(const std::string &path,
  ...
  if (header.magic != BIN_MAGIC) {
    return Result<ModelWeights>::err(
        "Invalid binary magic number: expected 0x" + std::to_string(BIN_MAGIC) +
  ...
  if (header.config.hidden_dim != config.hidden_dim ||
    ...
    return Result<ModelWeights>::err(
        "Model config mismatch: expected hidden_dim=" +
  ...
  size_t embed_size =
      static_cast<size_t>(config.vocab_size) * config.hidden_dim;
  ...
  for (int layer = 0; layer < config.num_layers; ++layer) {
    ...
    auto load_qweight = [&](QuantizedWeight &qw, int rows,
                            int cols) -> Result<void> {
  ...
  CUDA_CHECK(
      cudaMalloc(&weights.final_norm_weight, config.hidden_dim * sizeof(half)));
...
Result<QuantizedWeight> ModelLoader::loadQuantizedTensor(std::ifstream &file,
                                                         int rows, int cols,
                                                         int group_size) {
  ...
  file.read(reinterpret_cast<char *>(scale_host.data()),
            scale_size * sizeof(half));
  if (!file) {
  ...
  // Transfer to GPU
  CUDA_CHECK(cudaMemcpy(qw.data, weight_host.data(),
                        weight_size * sizeof(int8_t), cudaMemcpyHostToDevice));
