...
Result<void> GGUFParser::parse() {
  ...
  TLLM_INFO("GGUF header: version={}, tensors={}, metadata_entries={}",
            header_.version, header_.tensor_count, header_.metadata_kv_count);
...
Result<void> GGUFParser::parseHeader(std::ifstream &file) {
  ...
  if (header_.magic != GGUF_MAGIC) {
    return Result<void>::err(
        ...
        std::to_string(GGUF_MAGIC) + ", got 0x" + std::to_string(header_.magic));
  }
...
Result<void> GGUFParser::parseMetadata(std::ifstream &file) {
  for (uint64_t i = 0; i < header_.metadata_kv_count; ++i) {
    ...
    if (key_result.isErr()) {
      return Result<void>::err("Failed to read metadata key: " +
                               key_result.error());
    }
    ...
    if (value_result.isErr()) {
      return Result<void>::err("Failed to read metadata value for key: " +
                               key + ": " + value_result.error());
    }
...
Result<void> GGUFParser::parseTensorInfo(std::ifstream &file) {
  ...
  for (uint64_t i = 0; i < header_.tensor_count; ++i) {
    ...
    if (tensor_result.isErr()) {
      return Result<void>::err("Failed to read tensor info " +
                               std::to_string(i) + ": " + tensor_result.error());
    }

    tensors_.push_back(tensor_result.value());
    tensor_name_map_[tensors_.back().name] = tensors_.size() - 1;

    TLLM_TRACE("Tensor: {} dims={} type={} offset={}",
               tensors_.back().name, tensors_.back().dimensions.size(),
               static_cast<uint32_t>(tensors_.back().type),
...
Result<GGUFValue> GGUFParser::readArray(std::ifstream &file) {
  ...
  TLLM_TRACE("Reading array of {} elements, type {}", count,
             static_cast<uint32_t>(elem_type));
...
Result<ModelConfig> GGUFParser::extractModelConfig() const {
  ...
  if (config.hidden_dim <= 0 || config.num_layers <= 0 ||
      config.num_heads <= 0 || config.vocab_size <= 0) {
    ...
    if (config.intermediate_dim <= 0)
      config.intermediate_dim = config.hidden_dim * 8 / 3;
  }
...
GGUFParser::getTensorByName(const std::string &name) const {
...
GGUFParser::readTensorData(const GGUFTensorInfo &tensor) {
  std::ifstream file(path_, std::ios::binary);
  if (!file) {
  ...
  if (!file) {
    return Result<std::vector<uint8_t>>::err(
        "Failed to seek to tensor data at offset " + std::to_string(read_offset));
  }
...
size_t GGUFTensorInfo::calculateSize() const {
  ...
  default:
    ...
    TLLM_WARN("Unknown tensor type {}, assuming FP16 size",
