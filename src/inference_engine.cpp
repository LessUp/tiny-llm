...
Result<std::unique_ptr<InferenceEngine>> InferenceEngine::load(
...
InferenceEngine::~InferenceEngine() {
    if (hidden_states_) cudaFree(hidden_states_);
...
std::vector<int> InferenceEngine::generate(
    ...
    stats_.prefill_time_ms = std::chrono::duration<float, std::milli>(prefill_end - prefill_start).count();
...
void InferenceEngine::embedTokens(const int* tokens, int num_tokens, half* output) {
    ...
    CUDA_CHECK(cudaMemcpy(h_tokens.data(), tokens, num_tokens * sizeof(int), cudaMemcpyDeviceToHost));
    
    std::vector<half> h_output(num_tokens * config_.hidden_dim);
    for (int i = 0; i < num_tokens; ++i) {
        int token_id = h_tokens[i];
        if (token_id >= 0 && token_id < config_.vocab_size && weights_.token_embedding) {
            CUDA_CHECK(cudaMemcpy(
                output + i * config_.hidden_dim,
                weights_.token_embedding + token_id * config_.hidden_dim,
                config_.hidden_dim * sizeof(half),
...
void InferenceEngine::computeLogits(const half* hidden_states, int num_tokens, half* logits) {
    ...
    if (weights_.lm_head.isValid()) {
        kernels::w8a16_matmul(
            ...
            weights_.lm_head.scales,
            logits,
            num_tokens,
            config_.vocab_size,
            config_.hidden_dim,
            weights_.lm_head.group_size,
            stream_
        );
    }
}
...
int InferenceEngine::sampleTemperature(const half* logits, int vocab_size, float temperature, unsigned seed) {
...
// Top-k sampling
int InferenceEngine::sampleTopK(const half* logits, int vocab_size, int k, float temperature, unsigned seed) {
    // Get top-k indices
...
int InferenceEngine::sampleTopP(const half* logits, int vocab_size, float p, float temperature, unsigned seed) {
    // Sort by logit value
    std::vector<std::pair<float, int>> logit_pairs(vocab_size);
    for (int i = 0; i < vocab_size; ++i) {
        logit_pairs[i] = {__half2float(logits[i]), i};
