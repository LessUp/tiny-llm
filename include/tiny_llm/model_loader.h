...
private:
    ...
    static Result<std::pair<std::string, std::string>> readMetadataKV(std::ifstream& file);
    static Result<GGUFTensorInfo> readTensorInfo(std::ifstream& file);
    
    // Binary format helpers
    static Result<QuantizedWeight> loadQuantizedTensor(
    ...
    static Result<QuantizedWeight> allocateAndTransferQuantized(
        const int8_t* host_data, const half* host_scales,
