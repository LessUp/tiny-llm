...
public:
  ...
  void forward(half *hidden_states, KVCacheManager &kv_cache, int seq_id,
               int position, cudaStream_t stream = 0);

  // Forward pass for multiple tokens (prefill phase)
  // hidden_states: [batch_size, seq_len, hidden_dim] - input and output
  void forwardPrefill(half *hidden_states, KVCacheManager &kv_cache, int seq_id,
...
private:
  // Attention sublayer
  void attention(const half *x, half *output, KVCacheManager &kv_cache,
                 int seq_id, int position, int num_tokens, cudaStream_t stream);

  // Feed-forward network sublayer (SwiGLU)
  void feedForward(const half *x, half *output, int num_tokens,
                   cudaStream_t stream);
