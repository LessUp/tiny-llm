...
public:
  ...
  static void init(LogLevel level = LogLevel::INFO,
                   const std::string &log_file = "", bool async = false);
...
#define TLLM_TRACE(...)                                                        \
  SPDLOG_LOGGER_TRACE(tiny_llm::Logger::get(), __VA_ARGS__)
#define TLLM_DEBUG(...)                                                        \
  SPDLOG_LOGGER_DEBUG(tiny_llm::Logger::get(), __VA_ARGS__)
#define TLLM_INFO(...)                                                         \
  SPDLOG_LOGGER_INFO(tiny_llm::Logger::get(), __VA_ARGS__)
#define TLLM_WARN(...)                                                         \
  SPDLOG_LOGGER_WARN(tiny_llm::Logger::get(), __VA_ARGS__)
#define TLLM_ERROR(...)                                                        \
  SPDLOG_LOGGER_ERROR(tiny_llm::Logger::get(), __VA_ARGS__)
#define TLLM_CRITICAL(...)                                                     \
  SPDLOG_LOGGER_CRITICAL(tiny_llm::Logger::get(), __VA_ARGS__)
