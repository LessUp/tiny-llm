...
__global__ void rmsnorm_residual_kernel(const half *__restrict__ input,
...
void rmsnorm_residual(const half *input, const half *residual, const half *weight, half *output,
    ...
    rmsnorm_residual_kernel<<<batch_size, block_size, 0, stream>>>(input, residual, weight, output,
                                                                   hidden_dim, eps);
}
