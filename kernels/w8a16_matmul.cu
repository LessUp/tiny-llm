...
__global__ void dequantize_kernel(const int8_t *__restrict__ weight_int8,
                                  ...
                                  half *__restrict__ weight_fp16, int K, int N,
                                  int group_size) {
...
__global__ void fp16_matmul_kernel(const half *__restrict__ A,
...
void fp16_matmul_reference(const half *input, const half *weight, half *output,
  ...
  fp16_matmul_kernel<<<grid, block, 0, stream>>>(input, weight, output, M, N,
                                                 K);
}
...
__global__ void w8a16_matmul_reference_kernel(const half *__restrict__ input,
                                              ...
                                              const half *__restrict__ scales,
                                              half *__restrict__ output, int M,
