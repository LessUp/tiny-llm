...
__global__ void attention_decode_kernel(
    ...
    float scale, int num_heads, int seq_len, int head_dim) {
  ...
  const half *k =
      k_cache + (batch_idx * num_heads + head_idx) * seq_len * head_dim;
  const half *v =
      v_cache + (batch_idx * num_heads + head_idx) * seq_len * head_dim;
...
void attention_decode(const half *query, const half *k_cache,
...
__global__ void attention_prefill_kernel(const half *__restrict__ query,
                                         ...
                                         half *__restrict__ output, float scale,
  ...
  const half *q =
      query +
      ((batch_idx * num_heads + head_idx) * seq_len + query_pos) * head_dim;
  const half *k = key + (batch_idx * num_heads + head_idx) * seq_len * head_dim;
  const half *v =
      value + (batch_idx * num_heads + head_idx) * seq_len * head_dim;
  half *o =
      output +
      ((batch_idx * num_heads + head_idx) * seq_len + query_pos) * head_dim;
...
__global__ void get_attention_weights_kernel(
    ...
    half *__restrict__ weights, float scale, int num_heads, int query_len,
  ...
  const half *q =
      query +
      ((batch_idx * num_heads + head_idx) * query_len + query_pos) * head_dim;
  const half *k = key + (batch_idx * num_heads + head_idx) * key_len * head_dim;
  half *w =
      weights +
      ((batch_idx * num_heads + head_idx) * query_len + query_pos) * key_len;
...
void get_attention_weights(const half *query, const half *key, half *weights,
  ...
  get_attention_weights_kernel<<<grid, block_size, 0, stream>>>(
      query, key, weights, scale, num_heads, query_len, key_len, head_dim,
      apply_causal_mask);
}

// Simple softmax kernel
__global__ void softmax_kernel(const half *__restrict__ input,
                               half *__restrict__ output, int seq_len) {
  int row = blockIdx.x;
  int tid = threadIdx.x;
  int block_size = blockDim.x;
...
void softmax(const half *input, half *output, int batch_size, int seq_len,
  ...
  softmax_kernel<<<batch_size, block_size, shared_size, stream>>>(input, output,
                                                                  seq_len);
}
