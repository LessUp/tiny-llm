...
__global__ void
attention_decode_kernel(const half *__restrict__ query,   // [batch, num_heads, 1, head_dim]
...
__global__ void get_attention_weights_kernel(const half *__restrict__ query,
                                             const half *__restrict__ key,
                                             half *__restrict__ weights, float scale, int num_heads,
...
void get_attention_weights(const half *query, const half *key, half *weights, float scale,
                           int batch_size, int num_heads, int query_len, int key_len, int head_dim,
